    ...
    >
      ...
      >
        ...
        <div className="flex min-w-0 flex-1 items-center justify-center overflow-x-auto no-scrollbar sm:overflow-visible">
          <div className="flex min-w-0 flex-wrap items-center justify-center gap-x-0.5 gap-y-0.5 sm:rounded-xl sm:border sm:border-white/10 sm:bg-white/5 sm:px-1 sm:py-0.5 sm:backdrop-blur-md sm:min-w-0 sm:flex-nowrap sm:gap-1 sm:px-2">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
